...
function InnerStateChart({ entries }: { entries: JournalEntry[] }) {
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h3 className="text-sm font-medium mb-0.5" style={{ color: '#3C3C38' }}>How your mind has felt</h3>
        ...
        {bars.map(({ key, label, colour, bg }) => {
          ...
          return (
            <div key={key} className="flex items-center gap-3">
              <span className="text-xs w-14 shrink-0" style={{ color: '#8a8a82' }}>{label}</span>
              ...
              <span className="text-xs w-6 shrink-0 text-right tabular-nums" style={{ color: '#8a8a82' }}>
                {count}
              </span>
            </div>
          );
...
function BuildingState({ entryCount }: { entryCount: number }) {
  ...
  return (
    ...
    >
      <p className="text-base" style={{ color: '#8a8a82' }}>{message}</p>
    </motion.div>
  );
}
...
export function Insights({ entries }: InsightsProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="mb-8">
        <h2 className="text-2xl mb-2" style={{ color: '#3C3C38' }}>Insights</h2>
        <p style={{ color: '#6b6b64' }}>Patterns and observations from your entries</p>
      </div>
      ...
      {!sufficientData ? (
        ...
        <>
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <div className="p-2 rounded-lg" style={{ backgroundColor: 'rgba(0,0,0,0.04)' }}>
                          <Icon className="size-5" style={{ color: '#6b6b64' }} />
                        </div>
                        <div className="flex-1">
                          <p style={{ color: '#3C3C38' }}>{insight.text}</p>
                          <p className="text-sm mt-1 capitalize" style={{ color: '#8a8a82' }}>
          ...
          {insights.length === 0 && (
            <p className="text-sm mt-8" style={{ color: '#8a8a82' }}>
      ...
      {sufficientData && (
        <div className="mt-12 p-6 rounded-lg" style={{ backgroundColor: 'rgba(0,0,0,0.03)' }}>
          <h3 className="font-medium mb-2" style={{ color: '#3C3C38' }}>About Insights</h3>
